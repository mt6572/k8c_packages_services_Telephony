...
public class VTSettingUtils {
    ...
    public void updateVTSettingState(int slotId) {
        ...
        if (mPicToReplacePeer.equals(VTAdvancedSetting.SELECT_DEFAULT_PICTURE2)) {
            if (mToReplacePeer) {
                VTManagerWrapper.getInstance().setPeerView(1, VTAdvancedSetting.getPicPathDefault2());
            } else {
                VTManagerWrapper.getInstance().setPeerView(0, VTAdvancedSetting.getPicPathDefault2());
            }
        } else {
            if (mToReplacePeer) {
                VTManagerWrapper.getInstance().setPeerView(1, VTAdvancedSetting.getPicPathUserselect2(slotId));
            } else {
                VTManagerWrapper.getInstance().setPeerView(0, VTAdvancedSetting.getPicPathUserselect2(slotId));
            }
        }
        if (DBG) {
