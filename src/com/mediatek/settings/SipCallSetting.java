...
public class SipCallSetting extends android.app.Activity { 
    ...
    ///M: for adjust setting UI on VXGA device.
    public static class SipCallSettingFragment extends PreferenceFragment implements
            Preference.OnPreferenceClickListener, Preference.OnPreferenceChangeListener {
        WeakReference<SipCallSetting> instanceRef = null;

        @Override
        public void onCreate(Bundle savedInstanceState) {
            super.onCreate(savedInstanceState);
            instanceRef = new WeakReference<SipCallSetting>((SipCallSetting)getActivity());
            if (PhoneUtils.isVoipSupported()) {
                instanceRef.get().mSipManager = SipManager.newInstance(getActivity());
                instanceRef.get().mSipSharedPreferences = new SipSharedPreferences(getActivity());
                addPreferencesFromResource(R.xml.sip_settings_category);
                instanceRef.get().mButtonSipCallOptions = (CheckBoxPreference)this.findPreference("open_sip_call_option_key");
                instanceRef.get().mButtonSipCallOptions.setOnPreferenceClickListener(this);
                instanceRef.get().mListSipCallOptions = instanceRef.get().getSipCallOptionPreference();
                instanceRef.get().mAccountPreference = this.findPreference("sip_account_settings_key");
                instanceRef.get().mProfileDb = new SipProfileDb(getActivity());
                /// M: ALPS00507618 @{
                ...
                getPreferenceScreen().removePreference(instanceRef.get().mListSipCallOptions);
                instanceRef.get().mListSipCallOptions = null;
                /// @}
            }
        ...
        public boolean onPreferenceClick(Preference preference) {
            if (preference == instanceRef.get().mButtonSipCallOptions) {
                //handleSipCallOptionsChange(objValue);
                CheckBoxPreference cp = (CheckBoxPreference)instanceRef.get().mButtonSipCallOptions;
                ...
                new Thread(new Runnable() {
                    public void run() {
                        instanceRef.get().handleSipReceiveCallsOption(intEnable == 1);
                    }
                }).start();
                if (intEnable == 1) {
                    instanceRef.get().mAccountPreference.setEnabled(true);
                    if (instanceRef.get().mListSipCallOptions != null) {
                        instanceRef.get().mListSipCallOptions.setEnabled(true);
                    }
                } else {
                    instanceRef.get().checkAndSetDefaultSim();
                    instanceRef.get().mAccountPreference.setEnabled(false);
                    if (instanceRef.get().mListSipCallOptions != null) {
                        instanceRef.get().mListSipCallOptions.setEnabled(false);
                    }
                }
            }
            return false;
        ...
        public boolean onPreferenceChange(Preference preference, Object newValue) {
            ...
            if (preference == instanceRef.get().mListSipCallOptions) {
                instanceRef.get().handleSipCallOptionsChange(newValue);
            }
            return true;
        }
    }
