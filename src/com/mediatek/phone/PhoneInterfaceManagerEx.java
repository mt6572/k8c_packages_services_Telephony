...
/* 3G switch end */
    ...
    */
    ...
    //For Lte Dc feature -- start
    /**
     * Return current modem type.
     * @return current modem type.
     * @hide
     * @internal     
     */
    public int getActiveModemType() {
        log("[PhoneInterfaceManagerEx][getActiveModemType]");
        if (PhoneFactory.isLteSupport() && PhoneFactory.isWorldPhone()) {
    ...
    public int getCurrentRegion() {
        log("[PhoneInterfaceManagerEx][getCurrentRegion]");
        if (PhoneFactory.isLteSupport() && PhoneFactory.isWorldPhone()) {
            return LteWorldPhoneOp01.getCurrentRegion();
        }
        return -1;
    }

    public void setLteDcPhone(LteDcPhone phone){
        mLteDcPhone = phone;
    }

    // TODO: change name getServiceStateLteDc
    public Bundle getServiceStateLte(){
        Bundle data = new Bundle();
        mLteDcPhone.getServiceState().fillInNotifierBundle(data);
        return data;
    }

    public void setPreferredNetworkTypeLteDc(int networkType, Message response){
        mLteDcPhone.setPreferredNetworkType(networkType, response);
    }

    public void getPreferredNetworkTypeLteDc(Message response){
        mLteDcPhone.getPreferredNetworkType(response);
    }

    public String getNetworkOperatorLteDc(){
        return SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_NUMERIC_LTEDC);
    }

    public String getNetworkOperatorNameLteDc(){
        return SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_ALPHA_LTEDC);
    }

    public boolean isNetworkRoamingLteDc(){
        return SystemProperties.getBoolean(TelephonyProperties.PROPERTY_OPERATOR_ISROAMING_LTEDC,false);
    }

    public String getNetworkCountryIsoLteDc(){
        return SystemProperties.get(TelephonyProperties.PROPERTY_OPERATOR_ISO_COUNTRY_LTEDC);
    }

    //For Lte Dc feature -- end
}
