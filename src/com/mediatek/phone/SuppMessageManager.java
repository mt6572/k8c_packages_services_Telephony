    ...
    void doSuppCrssSuppServiceNotification(String number) {
        ...
        if (conn == null) {
            ...
            if (o instanceof CallerInfo) {
                ...
                if (!TextUtils.isEmpty(number) && !number.endsWith(ci.phoneNumber.replace(" ", "")) && !ci.isVoiceMailNumber()
                        && !ci.isEmergencyNumber()) {
                    ci.phoneNumber = number;
                    PhoneGlobals.getInstance().getCallModeler().onCrssSuppServiceNumberUpdate(conn, number);
                }
