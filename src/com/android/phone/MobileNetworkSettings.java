...
public class MobileNetworkSettings extends PreferenceActivity
    ...
    //sglte mode
    private SlteNetworkMode mSlteNetworkMode;
	private static final String BUTTON_SGLTE_NETWORK_MODE_EX_KEY = "button_sgltemode_ex_key";
	private Preference mButtonSglteNetworkModeEx;
    //UI objects
        ...
        public void onCreate(Bundle savedInstanceState) {
            ...
            if (getResources().getBoolean(R.bool.world_phone) == true) {
                ...
                /// @}
                    ...
                    /// @}
                    //instance.mExtension.customizeSgltePreferenceMode(getActivity(), getPreferenceScreen(), instance.mPhone);
                    instance.mSlteNetworkMode = new SlteNetworkMode();
                    instance.mSlteNetworkMode.customizeSgltePreferenceMode(getActivity(), getPreferenceScreen(), instance.mPhone);

                    
                    // add network mode for gemini
                    /*
                    Preference networkModeExPref = new Preference(this);
                    networkModeExPref.setKey(BUTTON_SGLTE_NETWORK_MODE_EX_KEY);
                    networkModeExPref.setTitle(R.string.gsm_umts_network_preferences_title);
					PreferenceScreen root = getPreferenceScreen();
					root.addPreference(networkModeExPref);
					instance.mButtonSglteNetworkModeEx = prefSet.findPreference(BUTTON_SGLTE_NETWORK_MODE_EX_KEY);
					*/
                } else {
    ...
    protected void onResume() {
        ...
        if (mButtonPreferredNetworkMode != null) {
            ...
            log ("MobileNetworkSettings::onResume: " + mButtonPreferredNetworkMode.getEntry());
        }

        if (mSlteNetworkMode != null) {
            log ("MobileNetworkSettings::onResume sglteGetPreferredNetworkTypeLteDc");
            mSlteNetworkMode.sglteGetPreferredNetworkTypeLteDc();
        }
    ...
    private class MyHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            log("[handleMessage]MyHandler msg.what = " + msg.what);
            switch (msg.what) {
    ...
    */
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_AIRPLANE_MODE_CHANGED)) {
                ...
                updateDataConnPref(true);
            } else if (action.equals(TelephonyIntents.ACTION_LTE_MD_TYPE_CHANGE)) {//csfb and mmdc changed
                if (FeatureOption.MTK_LTE_DC_SUPPORT) {
                    //5 is MD_TYPE_LWG----->CSFB
                    if (intent.getIntExtra(TelephonyIntents.INTENT_KEY_MD_TYPE, 0) == LteModemSwitchHandler.MD_TYPE_LWG) {
                        log("delete addVTSetting function.");
                        Preference sglteSingle = mFragment.getPreferenceScreen().findPreference("mmdc_mode");
                        if (sglteSingle != null) {
                            //mFragment.getPreferenceScreen().addPreference(sglteSingle);
                            mFragment.getPreferenceScreen().removePreference(sglteSingle);
                            log("Enter remove sglte single.");
                        }

                        Preference csfb = mFragment.getPreferenceScreen().findPreference("gsm_umts_preferred_network_mode_key");
                        if (csfb == null) {
                            mFragment.getPreferenceScreen().addPreference(mButtonPreferredNetworkMode);
                            log("Enter add csfb single.");
                        }
                    }
                    //6 is MD_TYPE_LTNG----->MMDC
                    if (intent.getIntExtra(TelephonyIntents.INTENT_KEY_MD_TYPE, 0) == LteModemSwitchHandler.MD_TYPE_LTNG) {
                        log("delete addVTSetting function.");
                        Preference sglteSingle = mFragment.getPreferenceScreen().findPreference("mmdc_mode");
                        if (sglteSingle == null) {
                            mFragment.getPreferenceScreen().addPreference(mSlteNetworkMode.mPrefereMode);
                            log("Enter remove sglte single.");
                        }

                        Preference csfb = mFragment.getPreferenceScreen().findPreference("gsm_umts_preferred_network_mode_key");
                        if (csfb != null) {
                            mFragment.getPreferenceScreen().removePreference(csfb);
                            log("Enter add csfb single.");
                        }
                    }
                }
    ...
    private void initCommonUI(PreferenceFragment fragment) {
        /// M: ALPS01444780 JE @{
        if (mFragment == null) {
            mFragment = fragment;
        }
