...
private  String getContactNameFromPhone( Context context, String phoneNum) {
        String contactName = "";  
        String callmode = SystemProperties.get("persist.service.callmode", "0");
	 if("1".equals(callmode))
	  return contactName;
	   ...
	   pCur.close();
	  //log("heyong contactName = " + contactName );
        return contactName;  
   }
    ...
    /* package */ Call onNewRingingConnection(Connection conn) {
       ...
       /*String contactName = ""; 
        String callmode = SystemProperties.get("persist.service.callmode", "0");
        //Log.i("heyong number  222= " + conn.getAddress() );
       final Phone phone = conn.getCall().getPhone();
       contactName = getContactNameFromPhone(phone.getContext(),conn.getAddress());
	 //Log.i("heyong Name 22= " + contactName );
       Log.d("heyong", "111 number: " + conn.getAddress()+"callmode = "+callmode);
	 if("1".equals(callmode)||(contactName==null)||"".equals(contactName))
	 	{
	 	}
	 else*/
	  if (PhoneGlobals.getInstance().getCallMode()==0)
    ...
    private void onDisconnect(Connection conn) {
	 ...
	 /* String contactName = ""; 
        String callmode = SystemProperties.get("persist.service.callmode", "0");
        //Log.i("heyong number  3333= " + conn.getAddress() );
	final Phone phone = conn.getCall().getPhone();
       contactName = getContactNameFromPhone(phone.getContext(),conn.getAddress());
	 //Log.i("heyong Name 333= " + contactName );
       Log.d("heyong", "number: " + conn.getAddress()+"callmode = "+callmode);
	 if("1".equals(callmode)||(contactName==null)||"".equals(contactName))
	 	{
	 	}
	 else */	
	 Log.d("heyong", "getCallMode: " +PhoneGlobals.getInstance().getCallMode());
	 if (PhoneGlobals.getInstance().getCallMode()==0)
        PhoneGlobals.getInstance().updateWakeState();
	  //PhoneGlobals.getInstance().setCallMode(0);
        Log.d("heyong", "2222getCallMode: " +PhoneGlobals.getInstance().getCallMode());
	...
	/*Connection c = (Connection) r.result;	
       ...
       String callmode = SystemProperties.get("persist.service.callmode", "0");
        //Log.i("heyong number  3333= " + conn.getAddress() );
	//final Phone phone = conn.getCall().getPhone();
       contactName = getContactNameFromPhone1(c .getAddress());
	 //Log.i("heyong Name 333= " + contactName );
      // Log.d("heyong", "number: " + conn.getAddress()+"callmode = "+callmode);
	 if("1".equals(callmode)||(contactName==null)||"".equals(contactName))
	 	{
	...
	//  {
	 ...
	 if (PhoneGlobals.getInstance().getCallMode()==0)
        ...
        /// @}
        PhoneGlobals.getInstance().notificationMgr.updateSpeakerNotification();
    }
        ...
        */
        ...
        for (Connection orphanedConnection : orphanedConnections) {
            ...
            if (mConfCallMap.containsKey(orphanedConnection)) {
                /// M: ALPS01432949 @{
                //  got NullPointException. should get call from mConfCallMap.
                //final Call call = mCallMap.get(orphanedConnection);
    ...
    public interface Listener {
        ...
        void onCrssSuppServiceNumberUpdate(int callId, String number);
        // For Dualtalk.
    ...
    public void onSeparateSipCallFailed(String errorMessage) {
        for (Listener listener : mListeners) {
            listener.onSuppServiceFailed(errorMessage);
        }
    }
    /// @}
    ...
    /// M: for ALPS01427149 @{
    // need to update contacts info when number changed because of COLP
    public void onCrssSuppServiceNumberUpdate(Connection conn, String number) {
        Call call = getCallFromMap(mCallMap, conn, false);
        if(call != null) {
            call.setNumber(number);
            int callId = call.getCallId();
            for (int i = 0; i < mListeners.size(); ++i) {
                mListeners.get(i).onCrssSuppServiceNumberUpdate(call.getCallId(), number);
            }
        }
    }
    /// @}
