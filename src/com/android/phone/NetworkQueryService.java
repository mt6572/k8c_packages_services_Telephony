...
public class NetworkQueryService extends Service {
    ...
    //mmdc begin...
    private static final int EVENT_NETWORK_SCAN_LTEMMDC_COMPLETED = 1000; //cmm mmdc
    private Phone mLteDcPhone;
    private boolean mLteDcPhoneComplete = false;
    private boolean mPhoneComplete = false;
    private ArrayList<OperatorInfo> mDcList = new ArrayList<OperatorInfo>();
    private ArrayList<OperatorInfo> mCList = new ArrayList<OperatorInfo>();
    private ArrayList<OperatorInfo> mDList = new ArrayList<OperatorInfo>();
    int mCEx = QUERY_OK;
    int mDEx = QUERY_OK;
    ...
    Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case EVENT_NETWORK_SCAN_LTEMMDC_COMPLETED:
                    if (!mLteDcPhoneComplete && !mPhoneComplete) mDcList.clear();
                    mLteDcPhoneComplete = true;
                    log("scan completed, broadcasting results EVENT_NETWORK_SCAN_LTEMMDC_COMPLETED");
                    synchronized (mCallbacks) {
                        mDList.clear();
                        broadcastQueryResultsForMmdc((AsyncResult) msg.obj, msg.what);
                    }
                    break;
                case EVENT_NETWORK_SCAN_COMPLETED:
                    if (FeatureOption.MTK_LTE_DC_SUPPORT) {
                        ...
                        //LteModemSwitchHandler.MD_TYPE_LWG is 5--->csfb 
                        //LteModemSwitchHandler.MD_TYPE_LTNG is 6 --->mmdc
                        PhoneInterfaceManagerEx temPhoneMgrEx = PhoneGlobals.getInstance().phoneMgrEx;;
                        state = temPhoneMgrEx.getActiveModemType();
                        if (state == -1) state = LteModemSwitchHandler.MD_TYPE_LTNG;
                        if (state == LteModemSwitchHandler.MD_TYPE_LTNG) {
                            log("scan completed, broadcasting results EVENT_NETWORK_SCAN_COMPLETED");
                            mPhoneComplete = true;
                            synchronized (mCallbacks) {
                                mCList.clear();
                                broadcastQueryResultsForMmdc((AsyncResult) msg.obj, msg.what);
                        ...
                        case QUERY_READY:
                            ...
                            */
                            ...
                            if (FeatureOption.MTK_LTE_DC_SUPPORT) {
                                ...
                                if (state == -1) state = LteModemSwitchHandler.MD_TYPE_LTNG;		
                                if (state == LteModemSwitchHandler.MD_TYPE_LTNG) {
                                    log("customizeMmdcPreferenceMode is mmdc");
                                    mLteDcPhone.getAvailableNetworks(mHandler.obtainMessage(EVENT_NETWORK_SCAN_LTEMMDC_COMPLETED));
    ...
    */
     ...
     * objects. 
     */
    private synchronized void broadcastQueryResultsForMmdc(AsyncResult ar, int what) {
        // reset the state.
        synchronized (mCallbacks) {
            mState = QUERY_READY;
            // see if we need to do any work.
            ...
            int exception = (ar.exception == null) ? QUERY_OK : QUERY_EXCEPTION;
            log("AsyncResult has exception " + exception + 
                " mPhoneComplete = " + mPhoneComplete +
                " mLteDcPhoneComplete = " + mLteDcPhoneComplete);
            if (ar.result != null) {
               for (OperatorInfo ni : ((ArrayList<OperatorInfo>)ar.result)) {
                    //default phone query over
                    if (EVENT_NETWORK_SCAN_COMPLETED == what) {
                        log("Default phone query result:");
                        mCList.add(ni);
                        log("  " + ni);
                        mCEx = exception;
                        log("default phone exception " + mCEx);
                    }
                    //lte phone query over
                    if (EVENT_NETWORK_SCAN_LTEMMDC_COMPLETED == what ) {
                        log("ltdDc phone query result:");
                        if (ni.getOperatorAlphaLong().contains("4")) {
                            mDList.add(0, ni);
                        } else {
                            mDList.add(ni);
                        }
                        log("  " + ni);
                        mDEx = exception;
                        log("ltdDc phone exception " + mDEx);
                    }
               }
            }

            if (mPhoneComplete && mLteDcPhoneComplete) {
                log("broadcastQueryResultsForMmdc next to broadcast and reset flag");
                mPhoneComplete = false;
                mLteDcPhoneComplete = false;
                String operatorNumeric = TelephonyManager.getDefault().getSimOperator();
                log("operatorNumeric" + operatorNumeric);
                mDcList.clear();
                for (OperatorInfo ni : mDList) {
                    if (ni.getOperatorNumeric().equals(operatorNumeric)) {
                        log("add to mDcList" + ni);
                        mDcList.add(ni);
                    } else {
                        continue;
                    }
            ...
            if (mDEx == QUERY_EXCEPTION && mCEx == QUERY_EXCEPTION) {
                exception = QUERY_EXCEPTION;
                log("AsyncResult lte and default has exception: " + exception);
            } else {
                exception = QUERY_OK;
                log("AsyncResult lte and default has exception: " + exception);
            }
            ...
            for (; i >= 0; i--) {
                ...
                try {
                    cb.onQueryComplete(mDcList, exception);			
                } catch (RemoteException e) {
                    log("e = " + e);
                }
            }   
