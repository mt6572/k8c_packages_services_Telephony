...
public class CallHandlerServiceProxy extends Handler
 implements CallModeler.Listener, AudioModeListener,
        ...
        PhoneRaiseDetector.Listener,CallCommandService.Listener {
    ...
    public CallHandlerServiceProxy(Context context, CallModeler callModeler,
        ...
        mCallCommandService.addListener(this);
    }
    ...
    private void processUpdate(List<Call> calls) {
        ...
        try {
            ...
            if (!mCallModeler.hasLiveCall() && getQueue().isEmpty()) {
...
//-------------------MTK----------------------------
    ...
    public void requestUpdateRecordState(final int state, final int customValue) {
        if (mCallHandlerServiceGuarded == null) {
            if (DBG) {
                Log.d(TAG, "CallHandlerService not connected.  Skip updateRecordState~~");
            }
            return;
        }
        Log.d(TAG, "[requestUpdateRecordState]do update");
     ...
     * This is done in CallHandlerService after related things has been done!
     * 
     * @see
        ...
        try {
            synchronized (mServiceAndQueueLock) {
                if (mCallHandlerServiceGuarded == null) {
                    if (DBG) {
                        Log.d(TAG, "CallHandlerService not conneccted. Skipping "
                                + "onCrssSuppServiceNumberUpdate...");
                    }
                    return;
                }
            }
            Log.d(TAG, "onCrssSuppServiceNumberUpdate, callId = " + callId + ", number =" + number);
            mCallHandlerServiceGuarded.onCrssSuppServiceNumberUpdate(callId, number);
        } catch (Exception e) {
            Log.e(TAG, "Remote exception handling onCrssSuppServiceNumberUpdate()", e);
        }
    }
}
