    ...
    public void placeCall(Intent intent) {
        ...
        switch (status) {
            ...
            case DROP_VOICECALL:
                ...
                if (bExtras == null) {
                    log("[placeCall]DROP_VOICECALL: intent contains no Extras, force set slotId to 0");
                }
                final int slot = bExtras == null ? 0 : bExtras.getInt(Constants.EXTRA_SLOT_ID);
	 ...
	 int mmicall = 0;
        switch (status) {
		 ...
		 Intent intent1 = new Intent();  
	       intent1.setAction( "android.intent.action.DIALED_MMI");
              intent1.setFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);	
	       intent1.putExtra("mmicall", 1);	
              PhoneGlobals.getInstance().sendBroadcast(intent1);
