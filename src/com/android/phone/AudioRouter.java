...
/* package */ class AudioRouter implements BluetoothIndicatorListener, WiredHeadsetListener {
    ...
    */
    ...
    public void onWiredHeadsetConnection(boolean pluggedIn) {
        ...
        /// M: for ALPS01256819 @{
        // when a second call incoming, the state is RINGING ,we still have to handle the headset event
        // final boolean isOffhook = (mCallManager.getState() == PhoneConstants.State.OFFHOOK);
        final boolean isPhoneInUse = (mCallManager.getState() != PhoneConstants.State.IDLE);
        /// @}
        ...
        /// M: For ALPS01316813. @{
        // when BT is already connected and the audio link is trying to
        // connect, we should not change the audio mode at here.
        if (!mBluetoothManager.isBluetoothHeadsetAudioOn() && mAudioMode != AudioMode.BLUETOOTH) {
        /// @}
    ...
    private int calculateModeFromCurrentState() {
        ...
        ///M: For ALPS01472177. Modify the order of caculation.
        // For the case of when the BluetoothIndication is showing
        // but the modem status is not really in BLUETOOTH mode. @{
        if (PhoneUtils.isSpeakerOn(mContext)) {
            mode = AudioMode.SPEAKER;
        } else if (mWiredHeadsetManager.isHeadsetPlugged()) {
            mode = AudioMode.WIRED_HEADSET;
        } else if (mBluetoothManager.showBluetoothIndication()) {
    ...
    private void updateAudioModeTo(int mode) {
        ...
        if (mAudioMode != mode) {
            ...
            doNotify = true;
            /// M: For ALPS01265905 @{
            // when AudioModeChanged, update the SpeakerNotification
