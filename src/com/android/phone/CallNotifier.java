...
import com.lava.phone.RecordListActivity;
        ...
        switch (msg.what) {
            ...
            case CallStateMonitor.PHONE_INCOMING_RING:
                ...
                if (msg.obj != null && ((AsyncResult) msg.obj).result != null) {
                    /// M: For For should send to voice mail and other should ignored incoming call. @{
                    // Original Code:
                    // PhoneBase pb =  (PhoneBase)((AsyncResult)msg.obj).result;
                    // if ((pb.getState() == PhoneConstants.State.RINGING)
                    //         && (mSilentRingerRequested == false)) {
                    Phone phone =  (Phone)((AsyncResult)msg.obj).result;
                    if ((phone.getState() == PhoneConstants.State.RINGING)
                            && (!mSilentRingerRequested) && !ignoreAllIncomingCalls(phone) && !mShouldSkipRing) {
                    /// @}
                        if (DBG) log("RINGING... (PHONE_INCOMING_RING event)");
						sendEmptyMessageDelayed(PHONE_RING_DELAY, 2000);
                        //mRinger.ring();
                    } else {
...
private  String getContactNameFromPhoneBook( String phoneNum) {
        String contactName = "";  
	String callmode = SystemProperties.get("persist.service.callmode", "0");
	 if(("1".equals(callmode))||(phoneNum==null)||"".equals(phoneNum))
		 	return contactName;
         ContentResolver cr = mApplication.getContentResolver(); 
    		...
    		if(phoneNum.length()>=8){
    			pCur = cr.query(  
        				...
        				new String[] {  "%" + phoneNum.substring(phoneNum.length()-8,phoneNum.length()) + "%" }, null);  
        		if (pCur.moveToFirst()) {  
    ...
    private void onNewRingingConnection(AsyncResult r) {
        ...
        String contactName = ""; 
        String callmode = SystemProperties.get("persist.service.callmode", "0");
        log("heyong number = " + c.getAddress() );
	...
	/* if("1".equals(SystemProperties.get("persist.service.callmode", "0")))
	 {
	      log("heyong Name =   " + SystemProperties.get("persist.service.callmode", "0"));
	      PhoneUtils.hangupRingingCall(ringing);
            return;
	 }*/
        ...
        if (PhoneUtils.isRealIncomingCall(state)) {
            ...
            /// M: For ALPS00569727 & ALPS00597749 @{
     ...
     */
     private static int send_mo_call = 0;
	...
	// intent.setAction( "android.intent.action.MO_CONNETCTED");
       // intent.setFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);
	 //intent.putExtra("k3callstate", state);	
     //   PhoneGlobals.getInstance().sendBroadcast(intent);
        ...
        /// M: For ALPS01476951. @{
        // no need do this at there because there has same handle when phone
        // state change in CallModeler.
        // Google code.
        /*
        ...
        */
        ...
        if (state == PhoneConstants.State.OFFHOOK) {
			...
			removeMessages(PHONE_RING_DELAY);
            mRinger.stopRing();
...
//                switchRingtoneForDualTalk(((CallNotifier) cookie).mRinger);
    ...
    /// @}
        ...
        Intent intent = new Intent();  
	 intent.setAction( "android.intent.action.MO_DISCONNETCTED");
        intent.setFlags(Intent.FLAG_INCLUDE_STOPPED_PACKAGES);
	if(send_mo_call == 1)
	 intent.putExtra("k3callstate",1);
	else
	  intent.putExtra("k3callstate",0);
	send_mo_call = 0;
        PhoneGlobals.getInstance().sendBroadcast(intent);
        ...
        /// M: For ALPS01447252 @{
        // WAITING to RINGING, setAudioMode() to RINGTONE_MODE for Voice UI (ALPS01447252).
        // Original Code:
        // if (mCM.getState() == PhoneConstants.State.IDLE) {
        if (mCM.getState() == PhoneConstants.State.IDLE || PhoneUtils.isOnlyRingingCallExist()) {
        /// @}
            ...
            if (((mPreviousCdmaCallState == Call.State.DIALING)
                ...
                if (!mIsCdmaRedialCall) {
                    if (autoretrySetting == InCallScreen.AUTO_RETRY_ON) {
                        // TODO: The contact reference data may need to be stored and use
        ...
        if (info != null) {
            ...
            if (info.isFinal) {
                ...
                if(c.getCall() != null && c.getCall().getPhone() != null){
                    n.slotId = c.getCall().getPhone().getMySimId();
                }
        ...
        if (FeatureOption.MTK_VT3G324M_SUPPORT) {
            if (c.isVideo()) {
                ...
                // Remember when single sim card, slot (sim) id is -1 !!!!!!!!
                        && (-1 == simId || simId == VTManagerWrapper.getInstance().getSimId())) {
                    if (!VTCallUtils.isVTActive()) {
                        ...
                        if (VTManager.State.CLOSE != VTManagerWrapper.getInstance().getState()) {
                            closeVTManager();
                        }
                    } else {
                        if (DBG)
     ...
     */
      public static final String mo_connetcted = "android.intent.action.MO_CONNETCTED";
    private void vibrateWhenMOConnected() {
        ...
        boolean isVibrate = sharedPreferences.getBoolean("button_connect_vibrate_key", false);
	...
	// PhoneGlobals.getInstance().sendBroadcast(new Intent(mo_connetcted));	  
        if(isVibrate){
		//lava add end by liuweibo 20140328
	...
	private static final int PHONE_RING_DELAY = 44;
    /**
