...
import android.os.Build;//lava add by liuweibo 20140328
/**
...
public class SpecialCharSequenceMgr {
    ...
    private static final String MMI_SW_VERSION_DISPLAY = "*#18375#";
    ...
    static boolean handleChars(Context context,
        ...
        if (handleIMEIDisplay(context, dialString)
			...
			||handleSARDisplay(context, dialString)//lava add by liuweibo 20140328
			|| handleSWDisplay(context,dialString)){//lava add by liuweibo 20140328
            return true;
        }
    ...
    static private boolean handleSecretCode(Context context, String input) {
        ...
        if (len > 8 && input.startsWith("*#*#") && input.endsWith("#*#*")) {//lava modify by liuweibo 20140328
            Intent intent = new Intent(TelephonyIntents.SECRET_CODE_ACTION,
                    Uri.parse("android_secret_code://" + input.substring(4, len - 4)));//lava modify by liuweibo 20140328
    ...
    static private boolean handlePinEntryGemini(Context context, String input,
        ...
        if ((input.startsWith("**04") || input.startsWith("**05"))
            ...
            boolean isMMIHandled = PhoneWrapper.handlePinMmi(app.phone, input, simId);
...
//lava add start by liuweibo 20140328
    static private boolean handleSARDisplay(Context context,
            String input){
    	if(input.equals(MMI_SAR_DISPLAY)){
    	    showSARPanel(context);
            return true;
    	}
    	   return false;
    }
    
    static private void showSARPanel(Context context) {
    	 AlertDialog alert = new AlertDialog.Builder(context)
         .setTitle("SAR")
         .setMessage("SAR < 1.6W/Kg")
         .setPositiveButton(R.string.ok, null)
         .setCancelable(false)
         .create();
         alert.show();
    ...
    static private boolean handleSWDisplay(Context context,
            String input){
    	if(input.equals(MMI_SW_VERSION_DISPLAY)){
    	    showSWPanel(context);
            return true;
    	}
    	   return false;
    ...
    static private void showSWPanel(Context context) {
    	 AlertDialog alert = new AlertDialog.Builder(context)
         .setTitle("SW VERSION")
