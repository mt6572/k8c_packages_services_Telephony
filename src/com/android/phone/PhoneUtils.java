            ...
            switch (msg.what) {
                ...
                case PHONE_STATE_CHANGED:
                    ...
                    for (Call fgCall : cm.getForegroundCalls()) {
                        if (fgCall != null && (fgCall.getPhone() instanceof SipPhone || !fgCall.isIdle())) {
                    ...
                    for (Call bgCall : cm.getBackgroundCalls()) {
                        if (bgCall != null && (bgCall.getPhone() instanceof SipPhone || !bgCall.isIdle())) {
    ...
    */
    ...
    static void checkCallKeyForVT(int phoneType) {
        ...
        if (!ignoreThisCallKey) {
            ...
            if (fgCall != null &&
                    (!(fgCall.getPhone() instanceof SipPhone) || !fgCall.getState().isDialing())) {
                ...
                Toast.makeText(PhoneGlobals.getInstance().getApplicationContext(),
                        R.string.incall_error_supp_service_switch, Toast.LENGTH_LONG).show();
            }
        }
    }
    ...
    public static void setANCEnable(boolean flag) {
        int status = 0;
        if (flag) {
            status = AudioSystem.setAudioCommand(SET_SPEECH_ANC_STATUS, ANC_ENABLE);
        } else {
            status = AudioSystem.setAudioCommand(SET_SPEECH_ANC_STATUS, ANC_DISABLE);
        }
    ...
    public static boolean isOnlyRingingCallExist() {
        boolean result = false;
        PhoneConstants.State state = PhoneGlobals.getInstance().mCM.getState();
        boolean hasActiveOrHoldCall = PhoneGlobals.getInstance().mCM.hasActiveFgCall()
                || PhoneGlobals.getInstance().mCM.hasActiveBgCall();
        if (PhoneConstants.State.RINGING == state && !hasActiveOrHoldCall) {
            result = true;
        }
        return result;
    }
